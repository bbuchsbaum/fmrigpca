...
test_that("build_spatial_metric creates valid column metric", {
  # Create test data
...
test_that("build_spatial_metric handles different tissue weights", {
  ...
  A1 <- build_spatial_metric(
    test_data$gm, test_data$wm, test_data$csf, L,
    mask_idx = mask_idx,
    alpha = 1.0, beta = 0.5, gamma = 1.0
  )
  
  A2 <- build_spatial_metric(
    test_data$gm, test_data$wm, test_data$csf, L,
    mask_idx = mask_idx,
...
test_that("build_spatial_metric handles missing tSNR", {
  test_data <- create_minimal_test_data(dims = c(6, 6, 4))
  L <- make_laplacian(test_data$mask, k = 4)
  mask_idx <- which(as.array(test_data$mask) != 0)
  
  # Without tSNR (should use default of 1)
  A_no_tsnr <- build_spatial_metric(
    test_data$gm, test_data$wm, test_data$csf, L,
  ...
  # With tSNR
  tsnr <- compute_tsnr(test_data$data)
  A_with_tsnr <- build_spatial_metric(
    test_data$gm, test_data$wm, test_data$csf, L,
    tsnr = tsnr, mask_idx = mask_idx
  )
...
test_that("build_spatial_metric validates tissue map consistency", {
  test_data <- create_minimal_test_data(dims = c(6, 6, 4))
  ...
  expect_error(
    build_spatial_metric(wrong_gm, test_data$wm, test_data$csf, L),
    "same NeuroSpace"
  )
})